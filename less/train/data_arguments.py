...
class DataArguments:
    ...
    max_seq_length: Optional[int] = field(
        default=256,
        metadata={
            "help": ("The maximum total input prompt length after tokenization. Sequences longer than this will be truncated,")
        },
    )
    max_res_length: int = field(
        default=256,
        metadata={
            "help": (
                'The maximum total completion length after tokenization. Sequences longer than this will be truncated'
            )
        }
    )
    ...
    data_dir: str = field(
        default="./data/data",
        metadata={"help": ("The directory where the data is stored.")},
    )
